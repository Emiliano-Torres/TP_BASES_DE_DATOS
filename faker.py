...
def load_faker():
    """Import the external faker package even though this script is faker.py."""
    script_dir = str(Path(__file__).resolve().parent)
    old_path = list(sys.path)
    sys.path = [
        path
        for path in sys.path
        if path not in ("", script_dir, str(Path.cwd().resolve()))
    ]
    try:
        return importlib.import_module("faker").Faker
    except ModuleNotFoundError as exc:
        raise SystemExit(
            "No se encontro la libreria faker. Instalacion sugerida: pip install Faker"
        ) from exc
    finally:
        sys.path = old_path
...
def make_countries():
    countries = []
    used_codes = set()
    for country_code in range(1, COUNTRY_COUNT + 1):
        country = fake.country()
        alpha_2 = fake.country_code()
        alpha_3 = alpha_2 + random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZ")
        while country_code in used_codes:
            country_code += 1
        used_codes.add(country_code)
        countries.append(
            (
                str(country_code),
                sql_text(country[:40]),
                sql_text(alpha_2[:2]),
                sql_text(alpha_3[:3]),
                sql_text(random.choice(["Americas", "Europe", "Asia"])),
                sql_text(random.choice(["South America", "Western Europe", "Eastern Asia"])),
                "NULL",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Genera datos Faker compatibles con pagila-schema.sql."
    )
    parser.add_argument(
        "-o",
        "--output",
        default="pagila-insert-data.sql",
        help="Archivo SQL de salida. Por defecto: pagila-insert-data.sql",
    )
    args = parser.parse_args()

    output = Path(args.output)
    country_ids = list(range(1, COUNTRY_COUNT + 1))
    ...
    with output.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("-- Datos generados por faker.py para pagila-schema.sql\n")
        ...
        for film_id in range(1, FILM_COUNT + 1):
            ...
            film_rows.append(
                (
                    ...
                    sql_text(random.choice(LANGUAGES)[0]),
                )
            )
    ...
    print(f"SQL generado en {output.resolve()}")
