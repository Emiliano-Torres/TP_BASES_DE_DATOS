...
Regenerar datos Pagila:

```powershell
python py\generar_pagila_faker.py
```

El comando genera `pagila-faker-test.sql` en la raiz del proyecto.
...
Los scripts `install-pagila.ps1` y `install-pagila.sh` copian `sql\pagila-schema.sql` y `pagila-faker-test.sql` al contenedor, recrean el schema `public`, crean las tablas e insertan los datos.
